...
- Pay close attention to the FAQ and don't promissse anything that's not explicitly written there.
...
def extract_prompt(metaprompt_response: str) -> str:
    ...
    cleaned = remove_empty_tags(between_tags[1000:].strip())
    return between_tags[:1000] + strip_last_sentence(cleaned.strip())
...
if prompt := st.chat_input("Describe your prompt"):
    ...
    with st.chat_message("assistant"):
        ...
        prompt = metaprompt.replace("{{TASK}}", prompt)
        assistant_partial = f"<Inputs>{variable_string}\n</Inputs>\n<Instructions Structure>"
        ...
        pretty_print = pretty_print(extracted_prompt_template)
        response = st.write(pretty_print)
